...
class ToolCatalogStartupLoggerTest {
    ...
    void shouldLogConfiguredToolsAndDiagnostics(CapturedOutput output) {
        ...
        properties.setRegistry(registryWith("tm_orders_search", "tm_knowledge_search"));
        ToolCatalog catalog = mock(ToolCatalog.class);
        when(catalog.discoveredToolProviders()).thenReturn(List.of(
                new ToolProviderDiagnostics("mcpProvider", List.of("tm_orders_search", "tm_internal_unconfigured"))
        ));
        ...
        when(catalog.duplicateDiscoveredToolNames()).thenReturn(List.of("tm_orders_search"));
        ToolCatalogStartupLogger startupLogger = new ToolCatalogStartupLogger(properties, catalog);

        startupLogger.logToolCatalog();

        assertThat(output)
                .contains("AI tool catalog configured tools: [tm_orders_search, tm_knowledge_search]")
                .contains("AI tool catalog discovered providers: [mcpProvider=[tm_orders_search, tm_internal_unconfigured]]")
                ...
                .contains("AI tool catalog duplicate discovered tool names: [tm_orders_search]");
    }

    @Test
    void shouldSkipEmptyDiagnosticWarnings(CapturedOutput output) {
        ToolPolicyProperties properties = new ToolPolicyProperties();
        properties.setRegistry(registryWith("tm_orders_search"));
        ToolCatalog catalog = mock(ToolCatalog.class);
        when(catalog.discoveredToolProviders()).thenReturn(List.of());
        when(catalog.discoveredToolNames()).thenReturn(List.of("tm_orders_search"));
        ...
        assertThat(output)
                .contains("AI tool catalog configured tools: [tm_orders_search]")
